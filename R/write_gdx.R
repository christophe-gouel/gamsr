prepare_4gdx <- function(x, symName, domains, symText, na.rm = TRUE) {
  if (na.rm) {
    nrow_x <- nrow(x)
    x <- na.omit(x)
    if (nrow(x) < nrow_x) warning("Rows with NAs have been removed")
  }
...
#'
#' @param x R variable (data.frame, vector, or factor) to be prepared for 
#' exportation.
...
#' @export
prepare_set4gdx <- function(x, symName, domains = "*", symText, na.rm = TRUE) {
  ...
  if (is.data.frame(x)) {
    ...
    stop("Input should be a data.frame, a vector, or a factor.")
  }
  prepare_4gdx(x, symName, domains, symText, na.rm = TRUE)
}
...
#' @return A 'data.frame' that can be used in the function 'write_gdx'.
#' @export
prepare_map4gdx <- function(x, symName, domains, symText, na.rm = TRUE) {
  if (missing(symName)) symName <- deparse(substitute(x))
...
#' @param symText optional character string defining the explanatory text of
#' the variable. Default to "".
#' @param na.rm logical. Should missing values be removed? Default to TRUE. For 
#' large variable without missing values, using to FALSE can result in 
#' significant speedup.
#' @return A 'data.frame' that can be used in the function 'write_gdx'.
#' @export
