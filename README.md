...
`gamsr` is just an opinionated interface to GAMS original package `gdxrrw` which has to be installed to use `gamsr`. Since `gdxrrw` is not on CRAN or a public git repo, the following function takes care of the installation.
